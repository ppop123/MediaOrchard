...
- Hardened `scripts/release_env_check.sh` so multi-machine release checks require `SHARED_ROOT_MARKER` and `SHARED_ROOT_MARKER_VALUE` by default; `REQUIRE_SHARED_ROOT_MARKER=0` is documented only for early diagnostics.
- Fixed Worker bootstrap for non-login SSH shells by exporting `/opt/homebrew/bin:/usr/local/bin:$PATH` before validating `ffmpeg` and `ffprobe`.
- Verified `bash scripts/verify.sh` and `bash scripts/release_check.sh` on `main` after the release gate and bootstrap PATH fixes: harness check, 140 passing tests, CLI smoke, `sdist`/wheel build, `twine check`, clean wheel install smoke, and tracked-file hygiene guard all pass.
- Created the NAS shared-root marker at `/Volumes/MediaOrchard/.mediaorchard-shared-root-id` and verified local, `192.168.50.8`, and `192.168.50.9` all read the same marker value.
- Executed `mediaorchard doctor worker-bootstrap --copy-wheel --execute` successfully for `192.168.50.8` and `192.168.50.9`; both target Worker venvs now import `mlx_whisper` 0.4.3, run `mediaorchard --help`, and pass `pip check`.
- Ran marker-verified `bash scripts/release_env_check.sh` on `main`; it exits `0` with local, `192.168.50.8`, and `192.168.50.9` passing Python, `ffmpeg`, `ffprobe`, `mlx_whisper`, shared root, shared-root marker, and bootstrap dry-run checks.
